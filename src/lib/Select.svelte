...
<div bind:this={select} class="flex flex-col flex-nowrap w-full relative">
    <button type="button" on:click={toggleSelect} class="py-2 px-3 relative text-left cursor-pointer bg-transparent w-full h-[42px] border {selected ? "border-slate-400" : "border-slate-400/40"} {invalid ? "border-red-500" : ""} rounded focus:border-sky-500 outline-none text-slate-400 focus:text-sky-500 peer group">
